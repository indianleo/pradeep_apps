...
let UIModal = React.createClass({
	propTypes : {
		...
		KeyboradTopOffses : PropTypes.number,
	...
	componentWillMount : function() {
		...
		if( UI.ios ) {
			this.subscriptions = [
				Keyborad.addListener( 'keyboardWillChnageFrame',this.onKeyboardChange ),
				Keyborad.addListener( 'keyboardDidHide', this.onKeyboardHide ),
			];
		}
	},
	...
	componentWillUnmount : function() {
		if( this.subscriptions ) {
			this.subscriptions.forech( (sub) => sub.remove() );
		}
	},
	...
	// keyboard is hidden in ios only 

	onKeyboardHide : function(event) {
     ...
     */
     ...
     animateBacdropOpen : function() {
     	...
     	this.state.animateBackdrop.start(()=>{
     		this.state.isAnimateBackdrop = false;
     	});
     },
